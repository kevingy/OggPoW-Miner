...
void EthGetworkClient::begin_connect()
{
    if (!m_endpoints.empty())
    {
        ...
        m_socket.async_connect(
            m_endpoint,
            m_io_strand.wrap(boost::bind(&EthGetworkClient::handle_connect, this,
        ...
        else
        {
            ...
            else
            {
               ...
               newWp.header = h256(JPrm.get(Json::Value::ArrayIndex(0), "").asString());
                ...
                {
                    std::string blkField = JPrm.get(Json::Value::ArrayIndex(3), "").asString();
                    newWp.block =
                        blkField.empty() ? -1 : (int)strtoul(blkField.c_str(), nullptr, 0);
                }
